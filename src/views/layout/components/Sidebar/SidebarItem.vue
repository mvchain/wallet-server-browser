<template>
  <div class="menu-wrapper">
    <template v-for="item in routes">
      ...
      <router-link v-if="(!item.hidden&&item.children&&item.children.length===1)&&item.meta.adminType.includes(adminType)"
      ...
      <el-submenu v-if="(!item.hidden&&item.children&&item.children.length>1)&&item.meta.adminType.includes(adminType)" :index="item.name||item.path"
                  :key="item.name">
        <template slot="title">
